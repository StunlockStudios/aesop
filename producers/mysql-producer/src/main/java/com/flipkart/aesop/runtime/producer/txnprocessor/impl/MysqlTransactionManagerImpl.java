...
/**
 * <code>MysqlTransactionManagerImpl</code> provides Mysql transaction lifecycle
 * management. Provides functionalities to begin, end and reset transactions and
 * begin and end sources.
 *
 * @author Shoury B
...
public class MysqlTransactionManagerImpl<T extends GenericRecord> implements MysqlTransactionManager {
	 ...
	 * @see
	 * com.flipkart.aesop.runtime.producer.txnprocessor.TransactionProcessor#endXtion(long)
	 */
	@Override
	...
	/**
	 * Resets the transaction in case of rollback. Resets the transaction
	 * state.
	 */
	@Override
	 ...
	 * @param databusOpcode operation code indicating nature of change such
	 * as insertion,deletion or updation.
	 */
	@Override
			...
			if (schema != null) {
				List<VersionedSchemaDbChangeEntry> entries
					= eventManagerMap.get(Integer.valueOf(41)).frameAvroRecord(
						eventHeader, rowList, databusOpcode, binLogEventMappers, schema,
						this.scnGenerator.getSCN(frameSCN(currFileNum, (int) eventHeader.getPosition()),
							this.mySqlEventProducer.getBinLogHost()));
				for (VersionedSchemaDbChangeEntry entry : entries) {
					perSourceTransaction.mergeDbChangeEntrySet(entry);
				}
				/**
				 * This is added here, because, if the
				 * mergePersource TXN is called during
				 * setSource, then , in the scenario where-in
				 * MysqlTransactionManagerImpl#perSourceTxn
				 * needs to be recycled, the DBEntry obtained
				 * above are not yet added to the new Instance
				 * of perSourceTransaction
				 */
				if (!perSourceTransaction.equals(transaction.getPerSourceTransaction(perSourceTransaction.getSrcId()))) {
					/**
					 * this If check signifies that the
					 * current per source txn is a source
					 * that we have already processed but we
					 * are receiving fresh events for it, so
	...
	/**
	 * Frames SCN from logid and offset. Lower 32 bits are offset and higher
	 * 32 bits are logid
	 *
	...
	/**
	 * Invoked by {@link MysqlTransactionManagerImpl#endXtion(long)}.Adds
	 * events to event buffer and saves the maximum SCN.
	 *
	...
	/**
	 * Invoked by
	 * {@link MysqlTransactionManagerImpl#onEndTransaction(Transaction)}.Adds
	...
	private void addTxnToBuffer(Transaction txn) throws DatabusException, UnsupportedKeyException,
		...
		for (PerSourceTransaction t : txn.getOrderedPerSourceTransactions()) {
			for (DbChangeEntry changeEntry : t.getDbChangeEntrySet()) {
				...
				try {
					length
						= eventManagerMap.get(41).createAndAppendEvent((VersionedSchemaDbChangeEntry)changeEntry, eventBuffer, false,
