...
import com.flipkart.aesop.runtime.producer.txnprocessor.impl.VersionedSchemaDbChangeEntry;
...
import com.stunlock.aesop.schemaregistry.ConfluentVersionedSchema;
import java.nio.ByteBuffer;

/**
 * <code>MysqlAvroEventManager</code> deals with avro events and provides avro
 * specific functionalities such as framing an avro record and appending avro
 * events to event buffer.
 *
 * <pre>
 * <ul>
...
public class MysqlAvroEventManager<T extends GenericRecord> {
	 ...
	 * @throws EventCreationException Thrown when event creation failed for
	 * a databus source
	 * @throws UnsupportedKeyException Thrown when the data type of the
	 * "key" field is not a supported type
	 * @throws DatabusException Generic Databus Exception
	 */
	public int createAndAppendEvent(VersionedSchemaDbChangeEntry changeEntry, DbusEventBufferAppendable eventBuffer,
		...
		/**
		 * Construct the Databus Event key, determine the key type and
		 * construct the key
		 */
		 ...
		 */
		byte[] globalIdBytes = ConfluentSchemaRegistryService.HackySchemaId;
		byte[] payload = serializeEvent(changeEntry.getRecord(), (ConfluentVersionedSchema)changeEntry.getVersionedSchema());
		DbusEventInfo eventInfo
			= new DbusEventInfo(changeEntry.getOpCode(), changeEntry.getScn(), (short) pSourceId, (short) pSourceId,
				changeEntry.getTimestampInNanos(), (short) lSourceId, globalIdBytes, payload,
				enableTracing, false);
	 ...
	 *
	 * @param eventHeader Binary log event header containing basic event
	 * details such as event type, event length etc. Sample :
	 ...
	 * @param dbusOpCode code indicating type of operation such as
	 * insertion, update or delete
	 * @param binLogEventMapper mapper corresponding to the source to which
	 * event belongs to
	 * @param schema schema corresponding to the source to which event
	 * belongs to
	 * @param scn system change number
	 * @return List<DbChangeEntry> list of change records
	 */
	public List<VersionedSchemaDbChangeEntry> frameAvroRecord(final BinlogEventV4Header eventHeader, final List<Row> rowList,
		final DbusOpcode dbusOpCode, Map<Integer, BinLogEventMapper<T>> binLogEventMappers, final VersionedSchema schema,
		final long scn) {
		List<VersionedSchemaDbChangeEntry> entryList = new ArrayList<VersionedSchemaDbChangeEntry>();
		...
		try {
			...
			for (Row row : rowList) {
				...
				GenericRecord genericRecord = binLogEventMapper.mapBinLogEvent(eventHeader, row, dbusOpCode, schema.getSchema());
				List<KeyPair> keyPairList = generateKeyPair(columns, schema.getSchema());
				VersionedSchemaDbChangeEntry dbChangeEntry
					= new VersionedSchemaDbChangeEntry(scn, timestampInNanos, genericRecord, dbusOpCode, isReplicated, schema,
						keyPairList);
	 ...
	 * a databus source
	 */
	protected byte[] serializeEvent(GenericRecord record,  ConfluentVersionedSchema versionedSchema) throws EventCreationException {
	...
	/**
	 * Generate Key pairs for the event. Gets primary key information from
	 * registered avro schema and generates key pairs for the given event
	 * details
	 *
	...
	private Object obtainKey(DbChangeEntry dbChangeEntry) throws DatabusException {
		...
		if (keyPairList.size() == 1) {
			...
			/**
			 * Treat multiple keys as a separate case to avoid
			 * unnecessary casts
			 */
			...
			while (li.hasNext()) {
				...
				if (li.hasNext()) {
					/**
					 * Add the delimiter for all keys except
					 * the last key
					 */
